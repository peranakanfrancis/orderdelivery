...
def insert_users(user_id, user_fname, user_lname, password, email, address, city, postal, apt, phone, memb_since, acc_funds):
    with sql.connect("losquatroamigos.db") as con:
        cur = con.cursor()
        cur.execute("INSERT INTO users (user_id,user_fname,user_lname,password,email,address, city, apt, postal,phone,memb_since,acc_funds) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                    (user_id, user_fname, user_lname, password, email, address, city, postal, apt, phone, memb_since, acc_funds) )
        con.commit()
...
def select_delivery_info():
    with sql.connect("losquatroamigos.db") as con:
        cur = con.cursor()
        result = cur.execute("SELECT users.user_fname, users.address, users.city, users.postal, users.phone FROM users "
